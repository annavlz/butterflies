...
describe('Custom Elements', function() {
  ...
  it('should recognize and create simple element that is registered', function(done) {
    ...
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('my-element', {key: 1})]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
  ...
  it('should render inner state and properties independently', function(done) {
    function myElementDef(ext) {
      ...
      return {DOM: Rx.Observable.combineLatest(ext.props.get('color'), number$, function(color, number) {
          return h('h3.stateful-element', {style: {color}}, String(number));
        })};
    }
    function app() {
      return {DOM: Rx.Observable.just('#00FF00').delay(50).startWith('#FF0000').map((color) => h('div', [h('my-element', {
          key: 1,
          'color': color
        })]))};
    }
  ...
  it('should have Observable properties object as props.get(\'*\')', function(done) {
    function myElementDef(ext) {
      return {DOM: ext.props.get('*').map((propsObj) => {
          ...
          return h('h3.inner-element', {style: {color: propsObj.color}}, String(propsObj.content));
        })};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div', [h('my-element', {
          color: '#FF0000',
          content: 'Hello world'
        })]))};
    }
  ...
  it('should have Observable properties object as props.getAll()', function(done) {
    function myElementDef(ext) {
      return {DOM: ext.props.getAll().map((propsObj) => {
          assert.strictEqual(typeof propsObj, 'object');
          assert.notStrictEqual(propsObj, null);
          assert.strictEqual(propsObj.color, '#FF0000');
          assert.strictEqual(propsObj.content, 'Hello world');
  ...
  it('should throw if properties driver getter has no args', function(done) {
    function myElementDef(ext) {
      return {DOM: ext.props.get().map((propsObj) => h('h3.inner-element', {style: {color: propsObj.color}}, String(propsObj.content)))};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div', [h('my-element', {
          color: '#FF0000',
    ...
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function onNextHandler() {
      assert.fail('DOM :root Observable should not emit onNext.');
    }, function onErrorHandler(err) {
      assert.strictEqual(err.message, 'Custom element driver `props.get()` ' + 'expects an argument in the getter.');
      done();
    });
  });
  it('should recognize and create two unrelated elements', function(done) {
    function myElementDef1() {
      return {DOM: Rx.Observable.just(h('h1.myelement1class'))};
    }
    function myElementDef2() {
      return {DOM: Rx.Observable.just(h('h2.myelement2class'))};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div', [h('my-element1'), h('my-element2')]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {
  ...
  it('should recognize and create a nested custom elements', function(done) {
    function innerElementDef() {
      return {DOM: Rx.Observable.just(h('h3.innerClass'))};
    }
    function outerElementDef() {
      return {DOM: Rx.Observable.just(h('div.outerClass', [h('inner-element', {key: 1})]))};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div', [h('outer-element', {key: 2})]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {
  ...
  it('should not render unwanted empty ids in elements', function(done) {
    function myElementDef() {
      return {DOM: Rx.Observable.just(h('h3.myelementclass', 'Hello world'))};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('my-element', {key: 1})]))};
    }
    ...
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
      ...
      assert.strictEqual(root.innerHTML, '<div class="toplevel">' + '<h3 class="myelementclass cycleCustomElement-MY-ELEMENT">' + 'Hello world' + '</h3>' + '</div>');
      responses.dispose();
      done();
    });
  });
  it('should not render unwanted leading spaces in className', function(done) {
    function myElementDef() {
      return {DOM: Rx.Observable.just(h('button', [h('p', "Some content")]))};
    }
    function main(responses) {
      return {DOM: responses.DOM.get('.target', 'click').startWith(0).scan((x, y) => x + 1, -1).map((number) => h('div', [h('p', `Counter: ${number}`), h('my-element.target')]))};
    }
    let [requests, responses] = Cycle.run(main, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
      assert.notStrictEqual(root, null);
      assert.notStrictEqual(typeof root, 'undefined');
      assert.strictEqual(root.innerHTML, '<div>' + '<p>Counter: 0</p>' + '<button class="target cycleCustomElement-MY-ELEMENT">' + '<p>Some content</p>' + '</button>' + '</div>');
      responses.dispose();
      done();
    });
  });
  it('should render custom element with a non-div top element', function(done) {
    function myElementDef({props}) {
      return {DOM: props.get('content').map((content) => h('h3.myelementclass', content))};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('p', 'Before'), h('my-element', {
          key: 1,
          content: 'Hello World'
        }), h('p', 'After')]))};
    }
  ...
  it('should catch custom element\'s interaction events', function(done) {
    function myElementDef() {
      return {
        ...
        events: {myevent: Rx.Observable.just(123).delay(300)}
      };
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('my-element.eventsource', {key: 1})]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
    Rx.Observable.combineLatest(responses.DOM.get(':root').skip(1).take(1), responses.DOM.get('.eventsource', 'myevent'), (root, event) => ({
      root,
      event
    })).subscribe(({root,
      event}) => {
  ...
  it('should not fail when finds VirtualText in replaceCustomElements', function(done) {
    ...
    function app() {
      return {DOM: Rx.Observable.just(h('h1', 'This will be a VirtualText'))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
  ...
  it('should not miss custom events from a list of custom elements #87', function(done) {
    function sliderDef(ext) {
      ...
      return {
        DOM: vtree$,
        events: {remove: remove$.withLatestFrom(id$, (r, id) => id)}
      };
    }
    function app(ext) {
      return {DOM: Rx.Observable.merge(Rx.Observable.just([{id: 23}]), Rx.Observable.just([{id: 23}, {id: 45}]).delay(50), ext.DOM.get('.slider', 'remove').map((event) => event.detail)).scan((items, x) => {
          if (typeof x === 'object') {
            return x;
          } else {
            return items.filter((item) => item.id !== x);
          }
        }).map((items) => h('div.allSliders', items.map((item) => h('slider-elem.slider', {id: item.id}))))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'slider-elem': sliderDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
  ...
  it('should recognize nested vtree as properties.get(\'children\')', function(done) {
    function simpleWrapperDef(ext) {
      return {DOM: ext.props.get('children').map((children) => {
          return h('div.wrapper', children);
        })};
    }
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('simple-wrapper', [h('h1', 'Hello'), h('h2', 'World')])]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'simple-wrapper': simpleWrapperDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
  ...
  it('should throw error if children property is explicitly used', function(done) {
    ...
    function app() {
      return {DOM: Rx.Observable.just(h('div.toplevel', [h('my-element', {children: 123})]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
    responses.DOM.get(':root').subscribeOnError((err) => {
      assert.strictEqual(err.message, 'Custom element should not have ' + 'property `children`. It is reserved for children elements nested ' + 'into this custom element.');
      responses.dispose();
      done();
    });
  });
  it('should recognize changes on a mutable collection given as props', function(done) {
    function xElementDef(ext) {
      return {DOM: ext.props.get('list', () => false).map((list) => h('div', [h('ol', list.map((value) => h('li.test-item', null, value)))]))};
    }
    let counter = 0;
    function app(ext) {
      let clickMod$ = ext.DOM.get('.button', 'click').map(() => `item${++counter}`).map((random) => function mod(data) {
        data.push(random);
        return data;
      });
      return {DOM: clickMod$.startWith([]).scan((data, modifier) => modifier(data)).map((data) => h('.root', [h('button.button', 'add new item'), h('x-element', {
          key: 0,
          list: data
        })]))};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'x-element': xElementDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
  ...
  it('should emit events even when dynamically evolving', function(done) {
    function myElementDef() {
      return {
        DOM: Rx.Observable.merge(Rx.Observable.just(h('h3.myelementclass', 'foo')), Rx.Observable.just(h('button.myelementclass', 'bar')).delay(50)),
        events: {myevent: Rx.Observable.just(123).delay(300)}
      };
    }
    function app() {
  ...
  it('should dispose vtree$ after destruction', function(done) {
    ...
    function myElementDef() {
      return {DOM: number$.do((i) => log.push(i)).map((i) => h('h3.myelementclass', String(i)))};
    }
    function app() {
      return {DOM: customElementSwitch$.map((theSwitch) => {
          return theSwitch === 0 ? h('div.toplevel', [h('my-element', {key: 1})]) : h('div.toplevel', []);
        })};
    }
    let [requests, responses] = Cycle.run(app, {DOM: makeDOMDriver(createRenderTarget(), {'my-element': myElementDef})});
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
  ...
  it('should not emit events after destruction', function(done) {
    ...
    function myElementDef() {
      return {
        ...
        events: {myevent: number$.do((i) => log.push(i))}
      };
    }
    function app() {
      return {DOM: customElementSwitch$.map((theSwitch) => {
    ...
    responses.DOM.get(':root').skip(1).take(1).subscribe(function(root) {
      ...
      assert.notStrictEqual(typeof myElement, 'undefined');
      assert.strictEqual(myElement.tagName, 'H3');
      myEventDisposable = Rx.Observable.fromEvent(myElement, 'myevent').take(3).subscribe(function(ev) {
